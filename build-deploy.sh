set -e
VERSION="test"
PLAYBOOK_FILE="./jicofo-build.yml"
LOCAL_BUILD="./target/jicofo-8.0-SNAPSHOT-jar-with-dependencies.jar"
REGION="us-east-1"
TAG_TYPE="jicofo"
...
aws s3 cp "${LOCAL_BUILD}" "s3://${BUCKET_LOCATION}/jicofo/${VERSION}/jicofo.jar"

PLAYBOOK="$(awk '{printf "%s\\n", $0}' $PLAYBOOK_FILE  | sed -e 's/"/\\"/g')"

PARAMETERS="{ \"playbook\": [\"${PLAYBOOK}\"],\"playbookurl\": [\"\"],\"extravars\": [\"BUCKET_LOCATION=${BUCKET_LOCATION} OBJECT_LOCATION=jicofo/${VERSION}/jicofo.jar\"],\"check\": [\"False\"],\"timeoutSeconds\": [\"3600\"]}"
